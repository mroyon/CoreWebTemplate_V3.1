...
{
    public sealed class Tran_LoginUseCase : ITran_LoginUseCase
    {
        ...
        public async Task<bool> GetAll(Tran_LoginRequest message, ICRUDRequestHandler<Tran_LoginResponse> outputPort)
        {
            ...
            try
            {
                IList<tran_loginEntity> oblist = await BFC.Core.FacadeCreatorObjects.Security.tran_loginFCC.GetFacadeCreate(_contextAccessor)
                .GetAll(new BDO.DataAccessObjects.SecurityModule.tran_loginEntity(), cancellationToken);
        ...
        public async Task<bool> GetListView(Tran_LoginRequest message, ICRUDRequestHandler<Tran_LoginResponse> outputPort)
        {
            ...
            try
            {
                ...
                if (oblist != null && oblist.Count > 0)
                {
                    outputPort.GetListView(new Tran_LoginResponse(oblist.ToList(), true));
                }
                else
                {
                    Tran_LoginResponse objResponse = new Tran_LoginResponse(false, _sharedLocalizer["NO_DATA_FOUND"], new Error(
        ...
        public async Task<bool> GetSingle(Tran_LoginRequest message, ICRUDRequestHandler<Tran_LoginResponse> outputPort)
        {
            ...
            try
            {
                tran_loginEntity objSingle = await BFC.Core.FacadeCreatorObjects.Security.tran_loginFCC.GetFacadeCreate(_contextAccessor)
        ...
        public async Task<bool> Save(Tran_LoginRequest message, ICRUDRequestHandler<Tran_LoginResponse> outputPort)
        {
            ...
            try
            {
                i = await BFC.Core.FacadeCreatorObjects.Security.tran_loginFCC.GetFacadeCreate(_contextAccessor)
                    .Add(message.Objtran_login, cancellationToken);
                outputPort.Save(new Tran_LoginResponse(true, _sharedLocalizer["DATA_SAVE_CONFIRMATION"], null));
                return true;
            }
            catch (Exception ex)
            {
        ...
        public async Task<bool> Update(Tran_LoginRequest message, ICRUDRequestHandler<Tran_LoginResponse> outputPort)
        {
            ...
            try
            {
                ...
                outputPort.Update(new Tran_LoginResponse(true, _sharedLocalizer["DATA_UPDATE_CONFIRMATION"], null));
                return true;
            }
            catch (Exception ex)
            {
        ...
        public async Task<bool> Delete(Tran_LoginRequest message, ICRUDRequestHandler<Tran_LoginResponse> outputPort)
        {
            ...
            try
            {
                ...
                outputPort.Delete(new Tran_LoginResponse(true, _sharedLocalizer["DATA_DELETE_CONFIRMATION"], null));
                return true;
            }
            catch (Exception ex)
            {
